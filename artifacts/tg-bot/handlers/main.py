...
# Словарь новых игроков для онбординг-напоминания: {user_id: datetime}
NEW_USERS: dict = {}
...
def get_business_menu() -> InlineKeyboardMarkup:
    ...
    buttons.append(
        [InlineKeyboardButton(text="📤 Продать бизнес", callback_data="sell_biz_select")]
    )
...
async def cmd_start(message: Message):
    ...
    if is_new:
        ...
        NEW_USERS[user_id] = datetime.now()
    ...
    onboarding_kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🍋 Купить Лимонадную ($50)", callback_data="buy_biz_lemonade")],
        [InlineKeyboardButton(
...
@router.callback_query(F.data == "sell_biz_select")
async def sell_biz_select(callback: CallbackQuery):
    """Показать список бизнесов для продажи"""
    user_id = callback.from_user.id
    businesses = db.get_user_businesses(user_id)

    if not businesses:
        await callback.answer("У вас нет бизнесов для продажи!", show_alert=True)
        return

    buttons = []
    for biz in businesses:
        biz_config = config.BUSINESSES.get(biz["business_type"], {})
        biz_level = biz["level"] if biz["level"] else 1
        sell_price = biz_config.get("base_cost", 0) * 0.5 * biz_level
        buttons.append([InlineKeyboardButton(
            text=f"📤 {biz_config.get('name', biz['business_type'])} Ур.{biz_level} → ${sell_price:.0f}",
            callback_data=f"sell_biz_confirm_{biz['id']}"
        )])

    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data="menu_business")])
    await callback.message.edit_text(
        "📤 <b>Продать бизнес</b>\n\n"
        "⚠️ Продажа за <b>50% цены</b>. Выберите бизнес:\n"
        "(уровень учитывается)",
...
@router.callback_query(F.data.startswith("sell_biz_confirm_"))
async def sell_biz_confirm(callback: CallbackQuery):
    """Подтвердить продажу конкретного бизнеса"""
    user_id = callback.from_user.id
    try:
        biz_id = int(callback.data.replace("sell_biz_confirm_", ""))
    except ValueError:
    ...
    db.remove_business(user_id, biz_id)
    db.update_balance(user_id, sell_price)
    new_balance = db.get_balance(user_id)

    await callback.message.edit_text(
        f"✅ <b>Бизнес продан!</b>\n\n"
        f"🏭 {biz_config.get('name', biz['business_type'])} (Ур.{biz_level})\n"
        f"💰 Получено: <b>${sell_price:.0f}</b>\n"
        f"💵 Баланс: <b>${new_balance:.2f}</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🏭 К бизнесам", callback_data="menu_business")],
            [InlineKeyboardButton(text="📊 Главное меню", callback_data="menu_main")],
        ])
    )
    await callback.answer(f"✅ Продано за ${sell_price:.0f}", show_alert=True)
...
async def lottery_play(callback: CallbackQuery):
    ...
    if not result["won"] and result["won_amount"] == 0:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="🔄 Ещё раз", callback_data=callback.data)],
                [InlineKeyboardButton(text="❌ Закрыть", callback_data="lottery_close")],
            ]
        )
    else:
...
async def onboarding_bonus(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🍋 Купить бизнес ($50)", callback_data="buy_biz_lemonade")],
        [InlineKeyboardButton(
